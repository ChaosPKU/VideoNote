...
(function($){
	...
	Redactor.prototype = {
		...
		setDoc: function(html)
		{
			...
			frameHtml += '<html><head><link media="all" type="text/css" href="'+'/profile/css/' + this.opts.css + '" rel="stylesheet"></head>';
		...
		{
			this.$editor.css({ 'min-height':this.$el.height()/3+'px' });
		...
		showDropDown: function(e, dropdown, key)
		{
			...
			var left = this.getBtn(key).offset().left-90;
		...
		showVideo: function()
		{
			...
			this.modalInit(RLANG.video, this.opts.path + '/plugins/video.html', 420, $.proxy(function()
			{
		...
		imageEdit: function(e)
		{
			...
			this.modalInit(RLANG.image, this.opts.path + '/plugins/image_edit.html', 420,  handler);
		...
		showImage: function()
		{
			...
			this.modalInit(RLANG.image, this.opts.path + '/plugins/image.html', 420, handler, true);
		...
		showLink: function()
		{
			...
			this.modalInit(RLANG.link, this.opts.path + '/plugins/link.html', 420, handler);
		...
		showFile: function()
		{
			...
			this.modalInit(RLANG.file, this.opts.path + '/plugins/file.html', 420, handler);
		},
		...
		modalInit: function(title, url, width, handler, scroll)
		{
			...
			$.ajax({
				...
				success: $.proxy(function(data)
				{
					...
					$('#redactor_modal').css({ width: width + 'px', height: 'auto', marginTop: '-' + (height+10)/2 + 'px', marginLeft: '-' + (width)/2 + 'px' }).fadeIn('fast');
